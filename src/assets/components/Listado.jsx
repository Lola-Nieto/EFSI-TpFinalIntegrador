...
function applyFilters(items, f) {
  ...
  if (f.type && f.type !== 'all') {
    out = out.filter(i => i.type === f.type)
  }
  if (f.cat && f.cat !== 'all' && f.cat.trim() !== '') {
    const c = f.cat.toLowerCase()
    out = out.filter(i => i.cat.toLowerCase() === c)
  }
  if (f.min !== '' && !Number.isNaN(Number(f.min))) {
    out = out.filter(i => i.amount >= Number(f.min))
  }
  if (f.max !== '' && !Number.isNaN(Number(f.max))) {
    out = out.filter(i => i.amount <= Number(f.max))
  }
  if (f.from) {
    out = out.filter(i => i.date >= f.from)
  }
  if (f.to) {
    out = out.filter(i => i.date <= f.to)
  }

  switch (f.order) {
    case 'date_asc':
      out.sort((a,b)=> a.date.localeCompare(b.date)); break
    case 'amount_desc':
      out.sort((a,b)=> b.amount - a.amount); break
    case 'amount_asc':
      out.sort((a,b)=> a.amount - b.amount); break
    case 'date_desc':
    default:
      out.sort((a,b)=> b.date.localeCompare(a.date)); break
  }
  ...
  return (
    <>
      ...
      <div className="card">
        ...
        {!data.length && (
          <p className="helper center" style={{marginTop:10}}>No hay resultados con los filtros aplicados.</p>
        )}
      </div>
    </>
  )
